...
class DBACaidDisplay(Poll, Converter, object):
			...
			if info:
				if info.getInfoObject(iServiceInformation.sCAIDs):
					...
					if ecm_info:
						...
						provid = provid.upper()
						provid = provid.zfill(4)
						provid = "PROVID: %s" % provid
						# hops
						...
						if using:
							if using == "emu":
								textvalue = "[EMU] - %s - %s - %s" % (caid, provid, ecm_time)
							elif using == "CCcam-s2s":
								textvalue = "[NET] - %s - %s - %s - %s - %s" % (address, caid, provid, hops, ecm_time)							
							else:
								textvalue = "[CARD] - %s - %s - %s - %s" % (address, caid, provid, ecm_time)		
						else:
							# mgcamd
