...
# coders by Nikolasi mod Ligioner 2012
...
class ExtremeInfo2(Poll, Converter,
 object):
    TUNERINFO = 0
    CAMNAME = 1
    NUMBER = 2
    ...
    def __init__(self, type):
        ...
        if (type == "TunerInfo"):
            ...
            self.type = self.MEMUSED
        elif (type == "CpuFree"):
            self.type = self.CPUFREE
        elif (type == "UsbInfo"):
    ...
    def getText(self):
        ...
        if ((self.type == self.TUNERINFO) or (self.type == self.TUNERINFOBP)):
            ...
            maxtemp = self.getLoadAvg()
            text = maxtemp
	elif (self.type == self.MEMTOTAL):
            memtotal = self.getMemTotal()
            text = memtotal
	elif (self.type == self.MEMFREE):
            memfree = self.getMemFree()
            text = memfree
	elif (self.type == self.MEMUSED):
            memused = self.getMemUsed()
            text = memused
	elif (self.type == self.CPUFREE):
            cpufree = self.getCpuFree()
            text = cpufree
...
#	loadg = "0"
	...
	memtl = "0"
	try:
		out_line = popen("cat /proc/meminfo").readline()
		memtl = "total: " + out_line[16:25]
		textvalue = memtl
	except:
		pass
        return textvalue
...
#	memfe = "0"
	try:
		out_line = popen("top -n 1|head -1|tail -1").readline()
		memfe = "free: " + out_line[19:26]
		textvalue = memfe
	except:
		pass
        return textvalue
...
#	cpufree = "0"
        data = [ ]
	try:
		file = os.popen("top -n 1|head -2|tail -1")
		data = file.split()[1]
		file.close() 
		textvalue = data
	except:
...
#	info = "0"
	try:
		out_line = popen("df -h | grep /media/usb").readline()
		info = out_line[0:76]
		textvalue = info
	except:
		pass
...
#       info = "0"
#	try:
#		out_line = popen("cat /proc/meminfo")
#		memfree = file.readlines()
#		out_line.close()
#	except:
#		print "error"
#	for line in memfree:
